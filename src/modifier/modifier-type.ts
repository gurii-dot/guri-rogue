...
// [MOD] 희귀 아이템 가중치 상향: MASTER x8, ROGUE x4, ULTRA x2
const tierWeights = [677 / 1024, 195 / 1024, 96 / 1024, 48 / 1024, 8 / 1024];
/**
...
export function getPlayerModifierTypeOptions(
  ...
  // [MOD] 메가 브레이슬릿은 80웨이브 이전, 메가스톤은 100웨이브 이전 확정 등장
  const currentWave = globalScene.currentBattle?.waveIndex ?? 0;
  const forceBeforeWave80 = currentWave > 0 && currentWave < 80;
  const forceBeforeWave100 = currentWave > 0 && currentWave < 100;
  const hasMegaBracelet = globalScene.modifiers.some(m => m instanceof MegaEvolutionAccessModifier);
  let forceSlot = options.length - 1;

  if (forceBeforeWave80 && !hasMegaBracelet && forceSlot >= 0) {
    const megaBraceletFunc = modifierTypeInitObj.MEGA_BRACELET;
    let megaType: ModifierType | null = megaBraceletFunc();
    if (megaType instanceof ModifierTypeGenerator) {
      megaType = megaType.generateType(party);
    }
    if (megaType) {
      options[forceSlot].type = megaType.withIdFromFunc(megaBraceletFunc).withTierFromPool(ModifierPoolType.PLAYER, party);
      forceSlot--;
    }
  }

  if (forceBeforeWave100 && hasMegaBracelet && forceSlot >= 0) {
    // 파티에서 메가진화 가능하지만 아직 메가스톤이 없는 첫 포켓몬과 그 메가스톤 아이템을 찾음
    let targetItem: FormChangeItem | undefined;
    for (const p of party) {
      if (!speciesDataRegistry.hasFormChanges(p.species.speciesId)) {
        continue;
      }
      const formChanges = speciesDataRegistry.getFormChanges(p.species.speciesId);
      const trigger = formChanges
        .filter(
          fc =>
            fc.preFormKey === p.getFormKey()
      ...
      const alreadyOwned = globalScene.findModifier(
        m =>
          m instanceof PokemonFormChangeItemModifier
          && m.pokemonId === p.id
          && m.formChangeItem === trigger.item,
      );
      if (!alreadyOwned) {
    ...
    if (targetItem !== undefined) {
      const rareFormChangeFunc = modifierTypeInitObj.RARE_FORM_CHANGE_ITEM;
      let stoneType: ModifierType | null = rareFormChangeFunc();
      if (stoneType instanceof ModifierTypeGenerator) {
        stoneType = stoneType.generateType(party, [targetItem]);
      }
      if (stoneType) {
        options[forceSlot].type = stoneType
          .withIdFromFunc(rareFormChangeFunc)
          .withTierFromPool(ModifierPoolType.PLAYER, party);
      }
    }
  }
...
function getNewModifierTypeOption(
  ...
  if (tier === undefined) {
    ...
    // [MOD] 희귀 아이템(ULTRA/ROGUE/MASTER) 등장 확률 상향에 맞춘 새 임계값
    if (tierValue > 346) {
      tier = ModifierTier.COMMON;
    } else if (tierValue > 151) {
      tier = ModifierTier.GREAT;
    } else if (tierValue > 55) {
      tier = ModifierTier.ULTRA;
    } else if (tierValue > 7) {
      tier = ModifierTier.ROGUE;
    } else {
