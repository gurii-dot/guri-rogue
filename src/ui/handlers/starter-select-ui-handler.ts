...
import { getStarterEggMoves, hasStarterEggMoveData } from "#utils/starter-customization";
import {
  ...
  */
  ...
  updateStarters = () => {
    ...
    this.validStarterContainers.forEach(container => {
      ...
      const isStarterProgressable = hasStarterEggMoveData(container.species.speciesId);
  ...
  setSpeciesDetails(species: PokemonSpecies, options: SpeciesDetails = {}, save = true): void {
    ...
    if (species) {
      ...
      if (dexEntry.caughtAttr) {
        ...
        if (hasStarterEggMoveData(species.speciesId)) {
          const speciesEggMoveList = getStarterEggMoves(species.speciesId);
          for (let em = 0; em < 4; em++) {
            if (starterDataEntry.eggMoves & (1 << em)) {
              this.speciesStarterMoves.push(speciesEggMoveList[em]);
            }
          }
        }
        ...
        const availableStarterMoves = this.speciesStarterMoves.concat(
          hasStarterEggMoveData(species.speciesId)
            ? getStarterEggMoves(species.speciesId).filter((_: MoveId, em: number) => starterDataEntry.eggMoves & (1 << em))
            : [],
        );
    ...
    const hasEggMoves = species && hasStarterEggMoveData(species.speciesId);
    let eggMoves = 0;
    if (species) {
    ...
    const speciesEggMoveList = species ? getStarterEggMoves(species.speciesId) : [];

    for (let em = 0; em < 4; em++) {
      const eggMove = hasEggMoves ? allMoves[speciesEggMoveList[em]] : null;
