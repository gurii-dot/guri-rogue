...
import { getSpeciesPassive, getStarterEggMoves, resolveAbilityIdByLocalizedName, resolveMoveIdByLocalizedName, setSpeciesPassive, setStarterEggMove } from "#utils/starter-customization";
...
import { PokedexScanEditType } from "#ui/pokedex-scan-edit-type";
...
export class PokedexPageUiHandler extends MessageUiHandler {
  ...
  private starterSetup(): void {
    ...
    this.eggMoves = getStarterEggMoves(this.starterId);
    ...
    this.passive = getSpeciesPassive(species.speciesId, this.formIndex);
  ...
  private createEggMoveOption(moveId: MoveId, slotIndex: number, style: TextStyle): OptionSelectItem {
    return {
      ...
      handler: () => {
        ...
        this.clearText();
        this.openEggMoveEditor(slotIndex);
        return true;
      },
  ...
  private openEggMoveEditor(slotIndex: number): void {
    const ui = this.getUi();
    const config = {
      buttonActions: [
        (sanitizedName: string) => {
          const name = decodeURIComponent(escape(atob(sanitizedName)));
          const moveId = resolveMoveIdByLocalizedName(name);
          if (moveId == null) {
            return;
          }
          setStarterEggMove(this.starterId, slotIndex, moveId);
          ui.revertMode();
    ...
    ui.setOverlayMode(UiMode.POKEDEX_SCAN, config, "", PokedexScanEditType.EGG_MOVE);
  }

  private openPassiveEditor(): void {
    const ui = this.getUi();
    const config = {
      buttonActions: [
        (sanitizedName: string) => {
          const name = decodeURIComponent(escape(atob(sanitizedName)));
          const abilityId = resolveAbilityIdByLocalizedName(name);
          if (abilityId == null) {
            return;
          }
          setSpeciesPassive(this.starterId, this.species.speciesId, this.formIndex, abilityId);
          ui.revertMode();
          ui.setMode(UiMode.POKEDEX_PAGE, "refresh");
        },
        () => {
    ...
    ui.setOverlayMode(UiMode.POKEDEX_SCAN, config, "", PokedexScanEditType.PASSIVE);
  }
              ...
              ui.showText(i18next.t("pokedexUiHandler:showEggMoves"), null, () => {
                ...
                ui.setModeWithoutClear(UiMode.OPTION_SELECT, {
                  options: [
                    ...
                    ...this.eggMoves.slice(0, 3).map((m, i) => this.createEggMoveOption(
                      m,
                      i,
                      this.hasEggMoves[i] ? TextStyle.SETTINGS_VALUE : TextStyle.SHADOW_TEXT,
                    )),
                    {
                    ...
                    this.createEggMoveOption(
                      this.eggMoves[3],
                      3,
                      this.hasEggMoves[3] ? TextStyle.SETTINGS_VALUE : TextStyle.SHADOW_TEXT,
                    ),
                    {
                      label: i18next.t("menu:cancel"),
                      handler: () => {
