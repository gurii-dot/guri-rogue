...
import { isPokedexScanEditType, PokedexScanEditType } from "#ui/pokedex-scan-edit-type";
import { getLocalizedAbilityNames, getLocalizedMoveNames } from "#utils/starter-customization";
...
export class PokedexScanUiHandler extends FormModalUiHandler {
  ...
  setup() {
    ...
    this.moveKeys = getLocalizedMoveNames();
    this.abilityKeys = getLocalizedAbilityNames();
  }

  getModalTitle(_config?: ModalConfig): string {
    if (this.row === PokedexScanEditType.EGG_MOVE) {
      return i18next.t("pokedexUiHandler:replaceEggMovePrompt");
    }
    if (this.row === PokedexScanEditType.PASSIVE) {
      return i18next.t("pokedexUiHandler:replacePassivePrompt");
    }
  ...
  override getInputFieldConfigs(): InputFieldConfig[] {
    switch (this.row) {
      case PokedexScanEditType.EGG_MOVE: {
        return [{ label: i18next.t("pokedexUiHandler:scanLabelMove") }];
      }
      case PokedexScanEditType.PASSIVE: {
    ...
    if (super.show(args)) {
      ...
      this.submitAction = () => {
        if (ui.getMode() === UiMode.POKEDEX_SCAN) {
          ...
          if (isPokedexScanEditType(this.row) && outputName === "") {
            return false;
          }
