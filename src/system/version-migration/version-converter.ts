...
function extractVersion(versionString: string): number[] {
  // Strip pre-release/build metadata suffixes (e.g. "1.11.18-aio", "1.2.3-beta")
  const coreVersion = versionString.trim().split("-")[0];
  // https://regex101.com/r/7r1299/1
  const regex = /^\d+\.\d+\.\d+(?:\.\d+)?$/;
  if (!regex.test(coreVersion)) {
    throw new Error(`Invalid version string (${versionString}) in version migrator!`);
  }

  const versionArray = coreVersion.split(".").map(v => Number.parseInt(v));
