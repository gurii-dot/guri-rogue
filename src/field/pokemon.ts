...
export abstract class Pokemon extends Phaser.GameObjects.Container {
  ...
  getUnlockedEggMoves(): MoveId[] {
    const moves: MoveId[] = [];
    const species =
      hasStarterEggMoveData(this.metSpecies) ? this.metSpecies : this.getSpeciesForm(true).getRootSpeciesId(true);
    if (hasStarterEggMoveData(species)) {
      const eggMoves = getStarterEggMoves(species);
      for (let i = 0; i < 4; i++) {
        if (globalScene.gameData.starterData[species].eggMoves & (1 << i)) {
          moves.push(eggMoves[i]);
        }
      }
    }
    return moves;
  ...
  getEggMoves(): MoveId[] | undefined {
    const rootSpeciesId = this.getSpeciesForm().getRootSpeciesId();
    if (!hasStarterEggMoveData(rootSpeciesId)) {
      return undefined;
    }
    return getStarterEggMoves(rootSpeciesId);
  }
