...
export class Egg {
  ...
  private rollEggMoveIndex() {
    ...
    if (!randSeedInt(baseChance)) {
      return 3;
    }

    // [MOD] 미해금 알기술(0~2번)이 남아있다면 그 중에서만 우선적으로 골라 해금되도록 함
    const unlockedMask = globalScene.gameData.starterData[this._species]?.eggMoves ?? 0;
    const lockedIndexes = [0, 1, 2].filter(i => !(unlockedMask & (1 << i)));
    if (lockedIndexes.length > 0) {
      return lockedIndexes[randSeedInt(lockedIndexes.length)];
    }
