...
export class SpeciesDataRegistry {
  ...
  public getStarterCost(speciesId: SpeciesId): StarterCost {
    ...
    // [MOD] 모든 포켓몬의 최대 코스트를 2로 제한.
    // 기존 코스트 감소 로직(1씩 감소, 1 이하에서는 절반)과 합쳐지면
    // 해금(캔디 소모)에 따라 2 -> 1 -> 0.5 순서로 코스트가 줄어듦.
    return Math.min(speciesData.starterCost as number, 2) as StarterCost;
  }
