...
import { getSpeciesPassive, getStarterEggMoves, hasStarterEggMoveData } from "#utils/starter-customization";
...
export abstract class PokemonSpeciesForm {
  ...
  getPassiveAbility(formIndex = this.formIndex): AbilityId {
    return getSpeciesPassive(this.speciesId, formIndex);
  }
  ...
  validateStarterMoveset(moveset: StarterMoveset, eggMoves: number): boolean {
    ...
    for (const moveId of moveset) {
      if (hasStarterEggMoveData(rootSpeciesId)) {
        const starterEggMoves = getStarterEggMoves(rootSpeciesId);
        const eggMoveIndex = starterEggMoves.indexOf(moveId);
